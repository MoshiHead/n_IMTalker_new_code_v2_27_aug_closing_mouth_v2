...
from mouth_closing import (
    ClosingAnimationLibrary,
    InterruptCloseLogger,
    MouthOpennessScorer,
    finalize_capture_clip,
    select_closing_clip,
)
...
class LiveHeliumFMOptions(BaseOptions):
    def initialize(self, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        ...
        parser.add_argument(
            "--enable_mouth_closing_interrupt",
            action="store_true",
            default=True,
            help="On barge-in, play a mouth-shape-matched closing animation before cutting to idle",
        )
        parser.add_argument(
            "--no_mouth_closing_interrupt",
            dest="enable_mouth_closing_interrupt",
            action="store_false",
            help="Disable the closing-animation interrupt feature (falls back to the instant idle cut)",
        )
        parser.add_argument(
            "--closing_animation_dir",
            default=str(ROOT / "closing_animations"),
            help="Directory to store captured closing-animation frame dumps for manual inspection",
        )
        parser.add_argument(
            "--interrupt_log_path",
            default=str(ROOT / "logs" / "interrupt_closing.log"),
            help="Dedicated log file for the interrupt/closing-animation subsystem",
        )
        parser.add_argument(
            "--closing_capture_max_frames",
            type=int,
            default=50,
            help="Max frames captured per natural speech->idle transition when building the closing library",
        )
...
def build_app(args: argparse.Namespace) -> FastAPI:
    ...
    # -- Natural-interrupt mouth-closing subsystem (process-global) --------
    mouth_closing_enabled = bool(getattr(args, "enable_mouth_closing_interrupt", True))
    interrupt_logger = InterruptCloseLogger(str(args.interrupt_log_path))
    mouth_scorer = MouthOpennessScorer()
    closing_library = ClosingAnimationLibrary(str(args.closing_animation_dir))
    closing_capture_pool = concurrent.futures.ThreadPoolExecutor(
        max_workers=2, thread_name_prefix="closing-capture"
    )
    closing_playback_pool = concurrent.futures.ThreadPoolExecutor(
        max_workers=1, thread_name_prefix="closing-select"
    )
    print(
        f"[INTERRUPT-CLOSE] enabled={mouth_closing_enabled} "
        f"library_dir={args.closing_animation_dir} log={args.interrupt_log_path}",
        flush=True,
    )
        ...
        try:
            while True:
                ...
                if packet.get("ws_kind") == "bytes":
                    ...
                    jb = packet.get("jpeg_bytes")
                    if jb:
                        session["last_sent_jpeg"] = jb
                else:
    ...
    async def conversation(ws: WebSocket):
        ...
        closing_animation_in_flight = False
        ...
        if reply_engine is not None:
            ...
            def _persona_priority_worker() -> None:
                ...
                async def _cancel_stale_media(generation: int) -> None:
                    nonlocal media_epoch, closing_animation_in_flight
                    dropped_audio = 0
                    dropped_frames = 0
                    while True:
                    ...
                    interrupt_logger.interrupt_detected(session_id, generation)
                    clip = None
                    closing_payload = None
                    if mouth_closing_enabled and not closing_animation_in_flight:
                        try:
                            last_jpeg = split_sessions.get(session_id, {}).get("last_sent_jpeg")
                            clip, score, bucket, reason = await event_loop.run_in_executor(
                                closing_playback_pool,
                                select_closing_clip,
                                last_jpeg, mouth_scorer, closing_library, interrupt_logger, session_id,
                            )
                            if clip is not None:
                                interrupt_logger.clip_selected(
                                    session_id, clip.clip_id, clip.bucket, len(clip.jpeg_frames), reason
                                )
                                closing_payload = {
                                    "clip_id": clip.clip_id,
                                    "bucket": clip.bucket,
                                    "frames": len(clip.jpeg_frames),
                                }
                            else:
                                interrupt_logger.fallback(session_id, f"no_clip_available({reason})")
                        except Exception as exc:  # noqa: BLE001 - never let this break the barge-in reset
                            interrupt_logger.error(session_id, "select_closing_clip", exc)
                            clip = None
                            closing_payload = None
                    elif mouth_closing_enabled:
                        interrupt_logger.fallback(session_id, "closing_animation_already_in_flight")

                    async with ws_send_lock:
                        await ws.send_json({
                            "type": "media_reset",
                            "generation": generation,
                            "reason": "native_personaplex_barge_in",
                            "closing_animation": closing_payload,
                        })

                    if clip is not None:
                        closing_animation_in_flight = True
                        t_play_start = time.perf_counter()
                        interrupt_logger.playback_start(session_id, clip.clip_id, len(clip.jpeg_frames))
                        frame_interval = 1.0 / float(args.fps)
                        frames_sent = 0
                        success = True
                        try:
                            for jb in clip.jpeg_frames:
                                if int(split_sessions.get(session_id, {}).get("media_generation", generation)) != generation:
                                    break
                                frame_start = time.perf_counter()
                                async with ws_send_lock:
                                    await ws.send_bytes(b"\x02" + jb)
                                frames_sent += 1
                                sleep_s = frame_interval - (time.perf_counter() - frame_start)
                                if sleep_s > 0:
                                    await asyncio.sleep(sleep_s)
                        except Exception as exc:  # noqa: BLE001
                            success = False
                            interrupt_logger.error(session_id, "closing_playback_send", exc)
                        duration_ms = (time.perf_counter() - t_play_start) * 1000.0
                        interrupt_logger.playback_end(session_id, clip.clip_id, success, duration_ms, frames_sent)
                        try:
                            async with ws_send_lock:
                                await ws.send_json({
                                    "type": "closing_animation_end",
                                    "clip_id": clip.clip_id,
                                    "success": success,
                                })
                        except Exception as exc:  # noqa: BLE001
                            interrupt_logger.error(session_id, "closing_animation_end_send", exc)
                        closing_animation_in_flight = False

                    interrupt_logger.idle_video_start(
                        session_id, "post_closing_animation" if clip is not None else "immediate"
                    )
            ...
            def _gpu_producer_thread() -> None:
                ...
                closing_capture_prev_active = True
                ...
                while not pipeline_stop_event.is_set():
                    ...
                    for ev in events:
                        ...
                        # Natural-interrupt capture: a speech->idle transition on
                        # this chunk carries the avatar's own organic mouth-close
                        # motion. Hand its JPEG frames to a background worker that
                        # scores/trims/saves it into the reusable closing library,
                        # off the hot render/enqueue path.
                        chunk_active_now = bool(fm_info.get("assistant_active", True))
                        if (
                            mouth_closing_enabled
                            and closing_capture_prev_active
                            and not chunk_active_now
                            and staged_frames
                        ):
                            capture_frames = [
                                pkt["jpeg_bytes"]
                                for pkt in staged_frames[: int(args.closing_capture_max_frames)]
                                if pkt.get("jpeg_bytes")
                            ]
                            closing_capture_pool.submit(
                                finalize_capture_clip,
                                capture_frames,
                                mouth_scorer,
                                closing_library,
                        ...
                        closing_capture_prev_active = chunk_active_now
